...
j1App::j1App(int argc, char* args[]) : argc(argc), args(args)
{
	...
	LOG("Constructor took %d ms", timer.Read());
}
...
bool j1App::Awake()
{
	...
	LOG("Awake took %d ms", timer.Read());
...
bool j1App::Start()
{
	...
	LOG("Start took %d ms", timer.Read());
	return ret;
}
...
{
	last_frame_ms_prepare_update = timer.Read();
}
...
void j1App::FinishUpdate()
{
	...
	uint32 last_frame_ms = (timer.Read() - last_frame_ms_prepare_update);
	...
	if (now_sec - last_sec == 1) {
		...
		frames_on_last_update = now_frame - last_frame;
	}
...
bool j1App::CleanUp()
{
	...
	LOG("CleanUp took %d ms", timer.Read());
